...
class FakeAggregate extends AggregateRoot<FakeIdentifier, FakeProps> {
...
describe('AggregateRoot - Testes', () => {
    it('deve adicionar eventos na lista', () => {
        ...
        aggregate.adicionarEvento({ nomeEvento: 'Evento', dataOcorrencia: new Date(), aggregateId: aggregate.id.valor, dados: { nome: 'ola' } })
        expect(aggregate.obterEventos()).toHaveLength(1)
    })
    it('deve obter a lista dos eventos', () => {
        const data = new Date(2022, 3, 4)
        const aggregate = new FakeAggregate(new FakeIdentifier('a'), { nome: 'b' });
        aggregate.adicionarEvento({ nomeEvento: 'Evento', dataOcorrencia: data, aggregateId: aggregate.id.valor, dados: { nome: '1' } })
        aggregate.adicionarEvento({ nomeEvento: 'Evento', dataOcorrencia: data, aggregateId: aggregate.id.valor, dados: { nome: '2' } })
        aggregate.adicionarEvento({ nomeEvento: 'Evento', dataOcorrencia: data, aggregateId: aggregate.id.valor, dados: { nome: '3' } })
        aggregate.adicionarEvento({ nomeEvento: 'Evento', dataOcorrencia: data, aggregateId: aggregate.id.valor, dados: { nome: '4' } })
